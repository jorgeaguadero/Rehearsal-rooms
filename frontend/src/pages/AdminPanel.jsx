...
function AdminPanel() {
  ...
  const handleDeleteRoom = async (id) => {
    setDeleteRoomId(id);
    setDeleteError("");
    // Comprobar reservas activas antes de mostrar el modal de confirmación
    try {
      const res = await api.get(`/api/rooms/${id}/availability`);
      const now = new Date();
      ...
      setDeleteError("No se pudo comprobar las reservas activas");
      setShowDeleteModal(true);
    }
  };
  ...
  const confirmDeleteRoom = async () => {
    ...
    try {
      ...
      setDeleteError(err.response?.data?.error || "Error al eliminar la sala");
    }
  };
        ...
        {showCreateForm ? (
          ...
          >
            ...
            <div className="flex gap-2 mt-2">
              <button
                type="submit"
                className="w-1/2 bg-[#68df9f] text-white p-2 rounded-md hover:bg-[#56df9e] font-semibold shadow-md transition"
              >
                Crear
              </button>
              <button
                type="button"
                className="w-1/2 bg-gray-400 text-white p-2 rounded-md hover:bg-gray-500 font-semibold shadow-md transition"
          ...
          <>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-semibold text-black">Salas</h2>
              <input
                type="text"
                placeholder="Buscar sala..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="border p-2 rounded text-black w-64 mr-4"
                style={{ maxWidth: 260 }}
              />
              <button
                className="bg-[#68df9f] text-white px-4 py-2 rounded-md hover:bg-[#56df9e] font-semibold shadow-md transition"
                  ...
                  >
                    {rooms
                      ...
                      .map((room) => (
                        <div
                          key={room.id}
                          className="bg-white p-4 rounded-lg shadow-md min-h-[420px] flex flex-col justify-between"
                          style={{ minWidth: 0 }}
                        >
                          {editId === room.id ? (
                            <form
